...
try:
    ...
    # 创建 900x383 的封面图 (微信推荐尺寸)
    img = Image.new('RGB', (900, 383), color='#1aad19')
    ...
    # 保存为临时文件
    cover_path = "C:/Users/wx151/wechat_cover_temp.jpg"
    img.save(cover_path, "JPEG", quality=95)
    print(f"✅ 封面图已生成：{cover_path}")
except Exception as e:
    print(f"⚠️ Pillow 不可用，将使用空封面：{e}")
    cover_path = None
...
# 步骤 3: 如果有封面图，先上传获取 media_id
thumb_media_id = ""
if cover_path and os.path.exists(cover_path):
    print("\n[3/4] 📤 正在上传封面图...")
    upload_url = f"https://api.weixin.qq.com/cgi-bin/media/upload?type=image&access_token={access_token}"
    ...
    if "media_id" in upload_data:
        ...
        print(f"✅ 封面图上传成功! Media ID: {thumb_media_id[:20]}...")
    else:
        print(f"⚠️ 封面图上传失败：{upload_data}")
        print("   将继续使用空封面")
...
if "media_id" in draft_data:
    ...
    print("=" * 60)
    print(f"\n📄 草稿 Media ID: {draft_data['media_id']}")
    print(f"\n🔗 查看地址:")
