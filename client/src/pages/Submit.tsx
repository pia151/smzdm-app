...
export default function Submit() {
  ...
  if (!user) {
    return (
      <div className="text-center py-20">
        ...
        <Link to="/login" className="inline-block bg-[#007AFF] text-white px-8 py-2.5 rounded-full text-sm">去登录</Link>
      </div>
    );
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      await api.submitDeal({
        ...form, price: Number(form.price),
        original_price: Number(form.original_price || 0),
        category_id: form.category_id ? Number(form.category_id) : null,
      });
      setSuccess('爆料成功！正在等待审核...');
      setForm({ title: '', content: '', price: '', original_price: '', platform: '', source_url: '', coupon_info: '', category_id: '' });
    } catch (err: any) {
  ...
  return (
    <div className="px-4 pt-4 pb-8">
      ...
      {error && <div className="bg-[#E8F0FE] text-[#007AFF] text-sm p-3 rounded-lg mb-4">{error}</div>}
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input type="text" value={form.title} onChange={e => setForm({...form, title: e.target.value})}
            ...
            className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-ios-500" required />
        </div>
        <div>
          <label className="text-xs text-gray-500 mb-1 block">描述</label>
          <textarea value={form.content} onChange={e => setForm({...form, content: e.target.value})}
            placeholder="描述一下这个好价，如活动范围、使用门槛等"
            className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-ios-500 min-h-[80px]" rows={3} />
        </div>
          ...
          <div className="flex-1">
            ...
            <div className="relative">
              ...
              <input type="number" value={form.price} onChange={e => setForm({...form, price: e.target.value})}
                placeholder="0.00"
                className="w-full border border-gray-200 rounded-lg pl-8 pr-3 py-2.5 text-sm focus:outline-none focus:border-ios-500" step="0.01" min="0" required />
            </div>
          </div>
          <div className="flex-1">
            ...
            <div className="relative">
              ...
              <input type="number" value={form.original_price} onChange={e => setForm({...form, original_price: e.target.value})}
                placeholder="0.00"
                className="w-full border border-gray-200 rounded-lg pl-8 pr-3 py-2.5 text-sm focus:outline-none focus:border-ios-500" step="0.01" min="0" />
            </div>
          </div>
        </div>
        <div className="flex gap-3">
          <div className="flex-1">
            ...
            <select value={form.platform} onChange={e => setForm({...form, platform: e.target.value})}
              className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-ios-500">
        ...
        <div>
          ...
          <input type="text" value={form.coupon_info} onChange={e => setForm({...form, coupon_info: e.target.value})}
