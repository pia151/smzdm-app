  ...
  if (error || !product) {
    return (
      <div className="text-center py-20 text-gray-400">
        ...
        <p>{error || '商品不存在'}</p>
        <Link to="/jd" className="text-red-500 text-sm mt-2 inline-block">返回京东精选</Link>
      </div>
    );
  }
    ...
    <div className="pb-4">
      ...
      <div className="px-4 pt-4">
          ...
          <div className="bg-gradient-to-r from-red-50 to-orange-50 rounded-lg p-3 mb-4 border border-dashed border-red-200">
            <div className="flex items-center justify-between">
              <span className="text-sm font-bold text-red-600">
                满{product.coupon.quota}减{product.coupon.discount}
              </span>
              <span className="text-xs text-red-400">优惠券</span>
            </div>
          </div>
        )}
        ...
        <div className="space-y-3 mb-4">
          {/* 购买按钮 */}
          {product.promotion_url && (
            <a
              href={product.promotion_url}
              target="_blank"
              rel="noopener noreferrer"
              className="block w-full bg-gradient-to-r from-red-500 to-orange-500 text-white text-center py-3 rounded-lg font-medium active:opacity-90"
            >
              去京东购买（带推广链接）
            </a>
          )}
          ...
          <button
            ...
            className="w-full py-2.5 rounded-lg text-sm font-medium border border-orange-200 bg-orange-50 text-orange-600 disabled:opacity-50"
          >
        ...
        {promotionUrl && (
          <div className="bg-green-50 rounded-lg p-3 mb-4 border border-green-100">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <button
                onClick={() => navigator.clipboard.writeText(promotionUrl)}
